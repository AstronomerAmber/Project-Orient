...
user_input = keras.layers.Input(shapef[1],name='User')
...
#rating = model.predict([X_test.user_id.iloc[2:5],X_test.item_id.iloc[13325:13328]])
#all_rates = model.predict([df_data.user_id, df_data.item_id])
#df_all_rates = pd.DataFrame(data=all_rating[0:,0:],index=idx[::],columns=all_rating[0,0:])
